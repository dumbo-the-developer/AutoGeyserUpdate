...
public class UpdateChecker implements Runnable {
    ...
    public static final long CHECK_UPDATE_FREQUENCY = 10; // 10 minutes

    /** The url to the jenkins last build information */
    public static final String LAST_BUILD_URL = "https://ci.opencollab.dev/job/GeyserMC/job/Geyser/job/master/lastSuccessfulBuild/api/json?random=%f";

    /** The url to download the geyser jar from */
    public static final String DOWNLOAD_URL = "https://ci.opencollab.dev/job/GeyserMC/job/Geyser/job/master/lastSuccessfulBuild/artifact/%s";

    private AutoGeyserUpdate autoGeyserUpdate;

    public UpdateChecker(AutoGeyserUpdate autoGeyserUpdate) {
        this.autoGeyserUpdate = autoGeyserUpdate;
    }
    ...
    public void run() {
        try {
            ...
            File oldJar = autoGeyserUpdate.getPlugin().getGeyserJar();
            if (oldJar == null || !oldJar.getName().equalsIgnoreCase(fileName)) {
                ...
                autoGeyserUpdate.startRestartCountdown();
            }


        } catch (Exception e) {
            System.err.println("[AutoGeyserUpdate] An error occured while checking for updates");
            e.printStackTrace();
        }

        autoGeyserUpdate.getPlugin().runTaskLaterAsync(this, CHECK_UPDATE_FREQUENCY);
    }
    ...
    private void installUpdate(File oldJar, String fileName, String relativePath) throws IOException {
        File newJar = new File(autoGeyserUpdate.getPlugin().getPluginsDirectory(), fileName);
